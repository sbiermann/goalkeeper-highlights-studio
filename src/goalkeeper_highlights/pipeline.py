...
def run(source: Path, output: Path, config: dict, overwrite: bool, ffmpeg: str = "ffmpeg", ffprobe: str = "ffprobe", progress_callback: ProgressCallback | None = None) -> dict[str, object]:
    ...
    try:
        ...
        if isinstance(precision_stats, dict):
            ...
            timings["requested_backend"] = str(precision_stats.get("requested_backend", "pytorch"))
            timings["effective_backend"] = str(precision_stats.get("effective_backend", "pytorch"))
            timings["backend_fallback_reason"] = precision_stats.get("backend_fallback_reason")
            timings["model_format"] = str(precision_stats.get("model_format", "pytorch"))
            timings["engine_cached"] = bool(precision_stats.get("engine_cached", False))
            timings["engine_build_seconds"] = float(precision_stats.get("engine_build_seconds", 0.0) or 0.0)
            timings["backend_load_seconds"] = float(precision_stats.get("backend_load_seconds", 0.0) or 0.0)
            timings["backend_warmup_seconds"] = float(precision_stats.get("backend_warmup_seconds", 0.0) or 0.0)
            timings["cuda_available"] = bool(precision_stats.get("cuda_available", False))
            timings["device"] = str(precision_stats.get("device", ""))
            timings["gpu_name"] = str(precision_stats.get("gpu_name", ""))
            timings["tensorrt_version"] = str(precision_stats.get("tensorrt_version", ""))
            timings["onnxruntime_version"] = str(precision_stats.get("onnxruntime_version", ""))
            timings["onnx_execution_provider"] = str(precision_stats.get("onnx_execution_provider", ""))
            timings["python_version"] = str(precision_stats.get("python_version", ""))
