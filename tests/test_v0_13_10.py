...
def test_v0_13_18_version_consistency():
    # Central source must be 0.13.24
    assert __version__ == "0.13.24"

def test_v0_13_18_cli_version():
    # CLI must report 0.13.24
    result = subprocess.run([sys.executable, "-m", "goalkeeper_highlights.cli", "--version"],
                            capture_output=True, text=True, check=True)
    # Output is usually "goalkeeper-highlights 0.13.24" or similar depending on prog name
    assert "0.13.24" in result.stdout
