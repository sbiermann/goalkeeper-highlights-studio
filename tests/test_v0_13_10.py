...
def test_v0_13_10_version_consistency():
    # Central source must be 0.13.10
    assert __version__ == "0.13.10"

def test_v0_13_10_cli_version():
    # CLI must report 0.13.10
    result = subprocess.run([sys.executable, "-m", "goalkeeper_highlights.cli", "--version"], 
                            capture_output=True, text=True, check=True)
    # Output is usually "goalkeeper-highlights 0.13.10" or similar depending on prog name
    assert "0.13.10" in result.stdout
